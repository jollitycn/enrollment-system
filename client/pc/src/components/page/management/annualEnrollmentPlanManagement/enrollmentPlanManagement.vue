  ...
  <div class="page-container">
    ...
    <div class="container">
      ...
      <el-table :data="tableData" style="width: 100%">
        ...
        <el-table-column prop="creatorName" label="制作人" width="180" align="center"></el-table-column>
        <el-table-column prop="createTime" label="制作日期" width="180" align="center">
          <!-- <template slot-scope="scope">{{createTime|resetTime}} </template> -->
        </el-table-column>
        <el-table-column prop="status" label="发布状态" width="180" align="center">
        ...
        <el-table-column prop="isDefaulted" label="默认状态" width="180" align="center">
        ...
        <el-table-column fixed="right" label="操作" width="420" align="center">
          <template slot-scope="scope">
            <el-button type="info" @click="copeOperation(scope.row.planId)">复制</el-button>
            <el-button type="success" @click="updatePlanStatus(2,scope.row.planId)" v-if="scope.row.status==1">发布
            </el-button>
            <el-button type="warning" @click="updateIsDefaulted(scope.row.planId)" v-if="scope.row.isDefaulted!=1 && scope.row.status==2">默认
            </el-button>
            <el-button type="primary" @click="createOrSee(1,scope.row.planId)">查看</el-button>
            <el-button type="primary" @click="createOrSee(2,scope.row.planId)" v-if="scope.row.status!=2">编辑</el-button>
            <el-button type="danger" @click="updatePlanStatus(0,scope.row.planId)" v-if="scope.row.status!=2">删除
            </el-button>
          </template>
        </el-table-column>
      </el-table>
    ...
    <div class="pagination">
      <el-pagination background layout="sizes, prev, pager, next,total" :current-page.sync="query.pageNum"
        :page-size="query.pageSize" :page-sizes="[10, 20, 50, 100]" @current-change="handlePageChange"
        @size-change="handleSizeChange"></el-pagination>
    </div>

    <el-dialog title="请选择学期" :visible.sync="dialogVisible" width="30%" :before-close="handleClose">
      <el-select placeholder="请选择学期" class="mr10" style="width: 100%;" v-model="copeForm.semester">
        <el-option @keyup.enter.native="handleSearch" v-for="item in semesterQuery" :key="item.name" :label="item.name"
          :value="item.name">
        </el-option>
      </el-select>
...
<script>
  ...
  import { addCollegeInfo } from '../../../../api/admin/enrollmentPlanManagement/addenrollmentPlan.js';
  import { listGroupInfoPage, updateIsDefaulted, updatePlanStatus, addCopyPlanInfo } from '../../../../api/admin/enrollmentPlanManagement/enrollmentPlan.js';
  import {
  getSemesterList,
  ...
  export default {
    ...
    methods: {
      ...
      //获取计划管理基本信息
      listGroupInfoPage() {
        listGroupInfoPage(this.query).then(res => {
      ...
      /**
       * 发布与删除的操作
       * @param status{int} 需要更改的状态（0：删除，1：未发布，2：已发布）
       ...
       */
      addCopyPlanInfo() {
        console.log(this.copeForm, '==>copeForm')
        addCopyPlanInfo(this.copeForm).then(res => {
          console.log(res, '--->copyres')
          let planId = res.data
      ...
      createOrSee(operation, planId) {
        this.$router.push({
          path: `/enrollmentPlanManagement/addenrollmentPlan/${operation}/${planId}`
        });
      },
      // 分页导航
