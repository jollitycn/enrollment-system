<template>
  <div class="student-target-add">
    <van-sticky :offset-top="0">
      <h5-header :leftStatus="true" title="新增目标生"></h5-header>
    </van-sticky>

    <van-form ref="stuForm" validate-first input-align="right">
      <!-- 学生姓名 -->
      <van-field
...
import { saveStudent } from "@/api/student/student-target-manage";
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        studentName: [{ required: true, message: "学校姓名不能为空" }],
        studentTelephone: [
          {
  ...
  created() {
    if (this.$route.params.cid) {
      this.form.classId = this.$route.params.cid;
    }
  },
  methods: {
    /**
