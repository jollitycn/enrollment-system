...
public class Swagger2 {
    ...
    public Docket createRestApi() {
        ...
        return new Docket(DocumentationType.SWAGGER_2).enable(true).apiInfo(apiInfo()).select()
                .apis(RequestHandlerSelectors.basePackage("com.educiot.recruit.server.wechat.controller")).paths(PathSelectors.any())
                .build().globalOperationParameters(par);
    }

    private ApiInfo apiInfo() {
        return new ApiInfoBuilder().title("育联网招生wechat 接口设计文档").version(apiVersion).build();
    }
