...
public class Swagger2 {
    ...
    public Docket createRestApi() {
        ...
        return new Docket(DocumentationType.SWAGGER_2).enable(true).apiInfo(apiInfo()).select()
                .apis(RequestHandlerSelectors.basePackage("com.educiot.recruit.server.app.controller")).paths(PathSelectors.any())
                .build().globalOperationParameters(par);
    }

    private ApiInfo apiInfo() {
        return new ApiInfoBuilder().title("育联网招生APP 接口设计文档").version(apiVersion).build();
    }
    ...
    public Docket planApi() {
        ...
        List<Parameter> par = new ArrayList<Parameter>();
        par.add(pb.build());//
        return new Docket(DocumentationType.SWAGGER_2).groupName("学生基本信息相关API").
                enable(true).
                apiInfo(apiInfo()).
                select()
                .apis(RequestHandlerSelectors.basePackage("com.educiot.recruit.server.app.controller.student")).paths(PathSelectors.any())
                .build().globalOperationParameters(par);
    }
