...
public class ConsultationServiceImpl extends ServiceImpl<ConsultationMapper, Consultation> implements IConsultationService {
     ...
     */
    @Override
    public Result<?> addOrEditConsultation(Long semesterId,String semester,Long userId,String userName, AddConsultationQuery addConsultationQuery) {
        ...
        if (addConsultationQuery.getConsultationId() == null) {
            ...
            //填充学期字段
            consultation.setSemester(semester);
            consultation.setSemesterId(semesterId);
            //填充创建信息
            consultation.setCreateTime(LocalDateTime.now());
            consultation.setCreatorId(userId);
            consultation.setCreatorName(userName);
