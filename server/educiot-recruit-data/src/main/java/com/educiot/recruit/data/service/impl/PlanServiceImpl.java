...
import com.educiot.common.api.DepartInfoResult;
...
public class PlanServiceImpl extends ServiceImpl<PlanMapper, Plan> implements IPlanService {
    ...
    @Override
    public Result addPlanInfo(AddPlanQuery addPlanQuery, Long recruitSchoolId, Long userId,String userName,Long apiId, APIRequest apiRequest) throws MyExcption {
        LocalDateTime now = LocalDateTime.now();
        if (null == addPlanQuery) {
            return Result.error(CodeMsg.PARAMETER_ISNULL);
        }
        addPlanQuery.setCreatorId(userId);
        addPlanQuery.setCreatorName(userName);
        Long schoolId = recruitSchoolId;
...
//        }
        ...
        try {
            ...
            planWrapper.eq("recruit_school_id",recruitSchoolId);
            ...
            addCollegeQuery.setRecruitSchoolId(recruitSchoolId);
            addCollegeQuery.setCreatorId(userId);
            addCollegeQuery.setCreatorName(userName);
            DepartInfoResult map = apiRequest.departInfo(apiId.toString());
    ...
    @Transactional(rollbackFor = Exception.class)
    @Override
    public Result updateStatus(UpdatePlanQuery updatePlanQuery,Long userId,String userName ) {
        LocalDateTime now = LocalDateTime.now();
        ...
        updatePlanQuery.setPublisherId(userId);
        updatePlanQuery.setPublisherName(userName);
        Plan plan = new Plan();
    ...
    @Transactional(rollbackFor = Exception.class)
    @Override
    public Boolean updateStatusEnd(UpdatePlanQuery updatePlanQuery, Long recruitSchoolId,Long currentSemesterId,Long userId,String userName) {
        Plan plan = new Plan();
        BeanUtils.copyProperties(updatePlanQuery, plan);

        //点击完成按钮，发送入学通知书
        taskReceiverService.sendAdmission(recruitSchoolId,currentSemesterId,userId,userName);
    ...
    @Transactional(rollbackFor = Exception.class)
    @Override
    public Result updateIsDefaulted(UpdatePlanIsDefaultecQuery updatePlanQuery, Long  recruitSchoolId) {
    ...
    @Transactional(rollbackFor = Exception.class)
    @Override
    public Result updatePlanInfo(UpdatePlanInfoQuery updatePlanInfoQuery,Long recruitSchoolId,Long  userId) {
        if (null == updatePlanInfoQuery) {
            return Result.error(CodeMsg.PARAMETER_ISNULL);
        }
        updatePlanInfoQuery.setModifierId(userId);
...
//        }
    ...
    @Transactional(rollbackFor = Exception.class)
    @Override
    public Result addCopyPlanInfo(AddCopyPlanQuery addCopyPlanQuery,Long recruitSchoolId,Long userId,String userName) {
        LocalDateTime now = LocalDateTime.now();
        if (null == addCopyPlanQuery) {
            return Result.error(CodeMsg.PARAMETER_ISNULL);
        }
        addCopyPlanQuery.setCreatorId(userId);
        addCopyPlanQuery.setCreatorName(userName);
        //根据copyId查询计划信息
