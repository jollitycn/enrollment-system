...
public class SysDictionaryServiceImpl extends ServiceImpl<SysDictionaryMapper, SysDictionary> implements ISysDictionaryService {
     ...
     */
    @Override
    public Result<?> addDictionary(AddDictionaryQuery addDictionaryQuery, LoginUser loginUser) {

        QueryWrapper<SysDictionary> wrapper = new QueryWrapper<>();
        wrapper.eq("data_code", addDictionaryQuery.getDataCode());
        wrapper.eq("is_deleted", Constant.SYS_ZERO);
        wrapper.eq("recruit_school_id", loginUser.getCurrentSchoolId());
        int count = count(wrapper);
        //判断数据字典编码是否有重复
        if (count > 0) {
        ...
        //获取招生学校ID
        QueryWrapper<RecruitSchool> recruitSchoolQueryWrapper = new QueryWrapper<>();
        recruitSchoolQueryWrapper.eq("api_id", loginUser.getApiInfo().getSchoolId());
        RecruitSchool recruitSchool = recruitSchoolService.getOne(recruitSchoolQueryWrapper);
        ...
        sysDictionary.setModifyId(loginUser.getApiInfo().getUserId());
        sysDictionary.setModifyName(loginUser.getApiInfo().getUserName());
     ...
     */
    @Override
    public Result<?> updateDictionary(UpdateDictionaryQuery updateDictionaryQuery, LoginUser loginUser) {
    ...
    @Override
    public List<SysDictVO> getAllDict(LoginUser loginUser) {
    ...
    @Override
    public List<SysDictionary> getDictListByCode(LoginUser loginUser, String dataCode) {

        //获取code对应的字典id
        QueryWrapper<SysDictionary> dictionaryQueryWrapper = new QueryWrapper<>();
        dictionaryQueryWrapper.eq("data_code", dataCode);
        dictionaryQueryWrapper.eq("recruit_school_id", loginUser.getCurrentSchoolId());
