...
public interface ITargetService extends IService<Target> {
     ...
     * @return 返回目标列表分页数据
     */
    IPage<TargetVO> getTargetListPage(ListTargetPageQuery param, LoginUser loginUser);

    /**
     * 添加或编辑招生目标
     *
     * @param addOrEditTargerReq 编辑招生目标参数封装
     * @param loginUser          登录用ku yv户信息
     * @return targetId
     * @throws Exception 初始化专业和渠道失败则会抛出此异常
     */
    Long addOrEditTarget(AddOrEditTargetReq addOrEditTargerReq, LoginUser loginUser, APIRequest apiRequest) throws Exception;
