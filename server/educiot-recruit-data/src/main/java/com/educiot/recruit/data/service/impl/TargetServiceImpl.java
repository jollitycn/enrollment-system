...
public class TargetServiceImpl extends ServiceImpl<TargetMapper, Target> implements ITargetService {
     ...
     */
    @Override
    public IPage<TargetVO> getTargetListPage(ListTargetPageQuery param, LoginUser loginUser) {

        //构建page对象
        Page<TargetVO> page = new Page<>(param.getPageNum(), param.getPageSize());

        //DAO查询，按创建时间倒序，时间格式化注解实现 格式为 YYYY-MM-DD HH:mm:ss
        IPage<TargetVO> result=baseMapper.getTargetListPage(page,loginUser.getCurrentSchoolId());
    ...
    public Long addOrEditTarget(AddOrEditTargetReq addOrEditTargerReq, LoginUser loginUser, APIRequest apiRequest)
            throws Exception {
        ...
        //获取学校id
        Long recruitSchoolId = loginUser.getCurrentSchoolId();

        //获取登录用户id
        Long userId = loginUser.getApiInfo().getUserId();

        //获取登录用户名
        String userName = loginUser.getApiInfo().getUserName();
