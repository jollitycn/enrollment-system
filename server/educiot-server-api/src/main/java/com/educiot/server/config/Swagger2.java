package com.educiot.server.config;
...
public class Swagger2 {
	...
	public Docket createRestApi() {
		...
		return new Docket(DocumentationType.SWAGGER_2).enable(true).apiInfo(apiInfo()).select()
				.apis(RequestHandlerSelectors.basePackage("com.educiot.server.controller")).paths(PathSelectors.any())
				.build().globalOperationParameters(par);
	}

	private ApiInfo apiInfo() {
		return new ApiInfoBuilder().title("招生接口设计文档").version(apiVersion).build();
	}
	...
	public Docket planApi() {
		...
		return new Docket(DocumentationType.SWAGGER_2).groupName("招生计划相关API").
				...
				apiInfo(apiInfo()).
				select()
				.apis(RequestHandlerSelectors.basePackage("com.educiot.server.controller.plan")).paths(PathSelectors.any())
				.build().globalOperationParameters(par);
	}
}
