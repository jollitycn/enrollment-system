...
class RedisUtil {
    ...
    public static void hmset(String key, Map<byte[], byte[]> paramMap) {
        ...
        try {
            ...
            log.error("往redis插入数据失败,key: {} and paramMap : {}", new Object[]{key, paramMap});
        } finally {
            returnResource(jedis);
        }
    }
    ...
    public static List<String> hmGet(String key, String... fields) {
        ...
        try {
            ...
            log.error("读取redis数据失败,key: {} and fields : {}", new Object[]{key, fields});
        } finally {
            returnResource(jedis);
        }
        return valueList;
    ...
    public static long getValidTime(String key) {
        ...
        try {
            ...
            log.error("获取redis-key有效时间失败,key: {}", key);
        } finally {
            returnResource(jedis);
        }
        return validTime;
...
//            value = jedis.set(key.getBytes(), SerializeUtil.serialize(value));
            ...
            value = jedis.set(key.getBytes(),value.toString().getBytes());
            jedis.expire(key.getBytes(), seconds);
        } catch (Exception e) {
    ...
    public static Long hdel(String[] keys) {
        ...
        try {
            ...
            log.error("根据key删除redis中的数据失败,keys: {}", keys);
        } finally {
            returnResource(jedis);
        }
...
//            for (field)
            ...
            log.error("根据key和fields删除redis中的数据失败,key: {}，fields：{}", key, fields);
        } finally {
            returnResource(jedis);
        }
...
//            Long result = jedis.hset(key.getBytes(), field.getBytes(), SerializeUtil.serialize(value));
            Long result = jedis.hset(key.getBytes(), field.getBytes(), value.toString().getBytes());
        } catch (Exception e) {
            log.error("hSet,key: {},value: {}", new Object[]{key, value});
        } finally {
            returnResource(jedis);
        }
    }
...
//            Long result = jedis.hset(key.getBytes(), field.getBytes(), SerializeUtil.serialize(value));
            ...
            log.error("hSetWithTime,key: {},value: {}", new Object[]{key, value});
        } finally {
            returnResource(jedis);
        }
    }
...
//            value = jedis.set(key.getBytes(), SerializeUtil.serialize(value));
            ...
            log.error("设置string数据类型数据,key: {},value: {}", new Object[]{key, value});
        } finally {
            returnResource(jedis);
        }
    }
